...
const mapInstance = ref(null); // Store the actual map instance
...
const hasActiveTrips = computed(() => {
  // Returns true if there is at least one active trip, false otherwise
...
<template>
  <div class="map-view-container bg-body p-4">
    ...
    <div class="map-area shadow-lg" :class="{ 'map-shifted': isDetailPanelOpen }">
      ...
      />
      <!-- TripReplayLayer outside of BaseMap, absolutely positioned -->
      <TripReplayLayer 
...
.sliding-detail-panel { position: absolute; top: 1rem; left: 362px; width: 422px; height: calc(100vh - 64px - 2rem); border-radius: var(--radius); background-color: transparent; z-index: 100; transform: translateX(-100%); opacity: 0; transition: all 0.3s ease-in-out; }
.sliding-detail-panel.panel-visible { transform: translateX(0); opacity: 1; }
.options-menu-overlay { position: absolute; top: 2rem; left: 0; right: 1.5rem; bottom: 0; height: calc(100vh - 64px - 4rem); background-color: transparent; z-index: 1000; display: flex; justify-content: flex-end; }
